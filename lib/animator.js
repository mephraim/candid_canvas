...
(function() {
  ...
  function _initForPlay(anim, options) {
    ...
    anim.setCurrentScene(anim.getCurrentScene() ||
    ...
    anim._playTimer = setInterval(function() {
      ...
      if (!anim.getCurrentScene()) {
        ...
        if (anim._looping) {
          ...
          anim.setCurrentScene(anim._remainingScenes.shift());
      ...
      if (anim._timeElapsedForCurrentScene < anim.getCurrentScene().duration) {
        _runScene(anim, anim.getCurrentScene(), anim._timeElapsedForCurrentScene);
        anim._timeElapsedForCurrentScene += frameDuration;
      // otherwise, load the next scene
      } else {
        anim.getCurrentScene().oncomplete();
        anim.setCurrentScene(anim._remainingScenes.shift());
        anim._timeElapsedForCurrentScene = 0;
      }
    }, frameDuration);
