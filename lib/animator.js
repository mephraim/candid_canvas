...
(function() {
  ...
  function _startFrameTimer(anim) {
    ...
    anim._playTimer = setInterval(function() {
      ...
      if (anim._timeElapsedForCurrentScene < anim.currentScene().duration) {
        _runScene(anim, anim.currentScene(), anim._timeElapsedForCurrentScene);
        anim._timeElapsedForCurrentScene += frameDuration;
      // otherwise, load the next scene
      } else {
        anim.currentScene().oncomplete();
        anim.currentScene(anim._remainingScenes.shift());
        anim._timeElapsedForCurrentScene = 0;
      }
    }, frameDuration);
